...
i32 simulate(instr_stream_t *stream_t, memory_t *mem) {
    cpu_t cpu = {0};
    ...
    instr_t instr_t;
    for (i32 stream_cursor = 0; stream_cursor < stream_t->count;) {
        instr_t = stream_t->stream[stream_cursor];
        print_instr(&instr_t, 0, 1);

        printf(" | IP: %#04x->", cpu.ip);
        cpu.ip += instr_t.size;
        printf("%#04x", cpu.ip);

        switch (instr_t.opcode) {
        case OP_NONE:
            fprintf(stderr, "; Unknown opcode make it to the simulation from decoding \
                    stage\n");
            return 1;
            break;
        case MOV_REG_MEM_REG:
        case MOV_REG_MEM_SEG:
            sim_mov_reg_mem(&cpu, &instr_t);
            stream_cursor++;
            break;
        case MOV_IMM_REG_MEM:
            sim_mov_imm_reg_mem(&cpu, &instr_t);
            stream_cursor++;
            break;
        case MOV_IMM_REG:
            sim_mov_imm_reg(&cpu, &instr_t);
            stream_cursor++;
            break;
        case ADD_REG_MEM_REG:
            sim_arithmetic_reg_mem(&cpu, &instr_t, binary_add, 1);
            stream_cursor++;
            break;
        case ADD_IMM_REG_MEM:
            sim_arithmetic_imm_reg_mem(&cpu, &instr_t, binary_add, 1);
            stream_cursor++;
            break;
        case ADD_IMM_ACC:
            sim_arithmetic_imm_acc(&cpu, &instr_t, binary_add, 1);
            stream_cursor++;
            break;
        case SUB_REG_MEM_REG:
            sim_arithmetic_reg_mem(&cpu, &instr_t, binary_sub, 1);
            stream_cursor++;
            break;
        case SUB_IMM_REG_MEM:
            sim_arithmetic_imm_reg_mem(&cpu, &instr_t, binary_sub, 1);
            stream_cursor++;
            break;
        case SUB_IMM_ACC:
            sim_arithmetic_imm_acc(&cpu, &instr_t, binary_sub, 1);
            stream_cursor++;
            break;
        case CMP_REG_MEM_REG:
            sim_arithmetic_reg_mem(&cpu, &instr_t, binary_sub, 0);
            stream_cursor++;
            break;
        case CMP_IMM_REG_MEM:
            sim_arithmetic_imm_reg_mem(&cpu, &instr_t, binary_sub, 0);
            stream_cursor++;
            break;
        case CMP_IMM_ACC:
            sim_arithmetic_imm_acc(&cpu, &instr_t, binary_sub, 0);
            stream_cursor++;
            break;
        case COND_JMP:
            sim_cond_jmp(&cpu, &instr_t, stream_t, &stream_cursor);
            break;
        case LOOP_JMP:
            break;
        default:
...
static void sim_mov_reg_mem(cpu_t *cpu, instr_t *i) {
...
static void sim_mov_imm_reg_mem(cpu_t *cpu, instr_t *i) {
...
static void sim_mov_imm_reg(cpu_t *cpu, instr_t *i) {
    operand_t *from = &i->operands[0];
    operand_t *to = &i->operands[1];
...
static void sim_mov_acc(cpu_t *cpu, instr_t *i) {}

static void sim_arithmetic_reg_mem(cpu_t *cpu, instr_t *instr_t,
                                   arithmetic arithmetic_func, i8 write_back) {
    operand_t *to = get_d(instr_t->ctrl_bits) ? &instr_t->operands[0]
                                              : &instr_t->operands[1];
    operand_t *from = get_d(instr_t->ctrl_bits) ? &instr_t->operands[1]
                                                : &instr_t->operands[0];
    bit_width_e bit_width_e =
        get_w(instr_t->ctrl_bits) ? BIT_SIZE_16 : BIT_SIZE_8;

    switch (get_m(instr_t->ctrl_bits)) {
    case MOD_11:
        assert(to->type == OPERAND_REG && from->type == OPERAND_REG);
        const u32 from_val = read_reg(cpu, from->reg);
        const u32 to_val = read_reg(cpu, to->reg);
        const u32 result =
            arithmetic_func(&cpu->flags, to_val, from_val, bit_width_e);
        if (write_back) {
    ...
    case MOD_10:
        printf(
            "Register arithmetic ops with memory access (16 bits offset) is yet \
                implemented\n");
        break;
    case MOD_01:
        printf(
            "Register arithmetic ops with memory access (8 bits offset) is yet \
                implemented\n");
        break;
    case MOD_00:
        printf(
            "Register arithmetic ops with memory access is yet implemented\n");
        break;
    }
}
...
static void sim_arithmetic_imm_reg_mem(cpu_t *cpu, instr_t *instr_t,
                                       ...
                                       i8 write_back) {
    operand_t *from = &instr_t->operands[0];
    operand_t *to = &instr_t->operands[1];

    imm_reg_mem_ops_assert(instr_t, from, to);

    bit_width_e bit_width_e =
        get_w(instr_t->ctrl_bits) ? BIT_SIZE_16 : BIT_SIZE_8;

    u32 from_val = from->word.value;
    if (get_s(instr_t->ctrl_bits) && get_w(instr_t->ctrl_bits) &&
        from_val >> 7 & 1) {
        from_val |= 0xff00;
    }
...
static void sim_arithmetic_imm_acc(cpu_t *cpu, instr_t *instr_t,
                                   arithmetic arithmetic_func, i8 write_back) {
    operand_t *reg_a_t = &instr_t->operands[0];
    operand_t *imm = &instr_t->operands[1];
    const bit_width_e bit_width_e =
        get_w(instr_t->ctrl_bits) ? BIT_SIZE_16 : BIT_SIZE_8;

    imm_acc_ops_assert(instr_t, reg_a_t, imm);

    const u32 reg_a_val = read_reg(cpu, reg_a_t->reg);
    const u32 result =
        arithmetic_func(&cpu->flags, reg_a_val, imm->word.value, bit_width_e);
    if (write_back) {
...
static void sim_cond_jmp(cpu_t *cpu, instr_t *i, instr_stream_t *stream_t,
                         i32 *stream_cursor) {
    i32 new_ip = 0;
    switch ((cond_jmp_e)decode_cond_jump_variant(i->raw_ctrl_bits1)) {
    case COND_JMP_NE:
        if (!(get_zf(cpu->flags))) {
    ...
    fprintf(stderr,
            "Failed to find instruction with base address that matches new IP "
            "value %#04x\n",
            new_ip);
    assert(0);
}
